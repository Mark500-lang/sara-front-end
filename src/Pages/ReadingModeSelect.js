...
const ActionIcons = ({
  ...
  const handleIconClick = (icon) => {
    ...
    if (icon === "read" || icon === "record") {
      if (onModeSelect) onModeSelect(icon);
    }
    // "listen" stays in overlay — user picks narration from the panel
  };

  const handleRecordButtonClick = () => {
    if (audioURL) onShowContinueModal();
  ...
  return (
    <div className={`action-icons ${overlayMode ? "overlay-style" : ""}`}>
      ...
      <motion.div className="action-header-container" initial="hidden" animate="visible" variants={headerVariants}>
      ...
      <motion.div className="select-mode-container" variants={containerVariants} initial="hidden" animate="visible">
        <motion.div className="action-icons-container" variants={modeButtonsVariants}
          animate={selectedMode ? "withDialogue" : "visible"}>

          <motion.div className={`mode-button ${selectedMode === "read" ? "clicked-mode-btn" : ""}`}
          ...
          <motion.div className={`mode-button ${selectedMode === "listen" ? "clicked-mode-btn" : ""}`}
          ...
          <motion.div className={`mode-button ${selectedMode === "record" ? "clicked-mode-btn" : ""}`}
        ...
        <AnimatePresence mode="wait">
          {selectedMode && (
            <motion.div className="select-mode-dialogue-box" variants={dialogueVariants}
              initial="hidden" animate="visible" exit="exit" key={selectedMode}>
              ...
              {selectedMode === "record" && (
                <>
                  <motion.div className="recordings-box" variants={itemVariants}>
                    ...
                    />
                    {isRecording ? (
                      <button className="record-button active" onClick={onStopRecording}>Stop</button>
                    ) : (
                      <button className={`record-button ${narratorName.trim() ? "active" : ""}`}
                        disabled={!narratorName.trim()} onClick={handleRecordButtonClick}>
                        Start
                      </button>
                    )}
                  </motion.div>
                  ...
                  <motion.div className="my-voice-over" variants={itemVariants}>
                    ...
                    {incompleteNarrators.length > 0 ? (
                      incompleteNarrators.map(({ narrator, page_count }) => (
                        <motion.div className="audio-item" key={narrator} variants={itemVariants}>
                          {/* ── RESUME FIX: clicking draft navigates AND auto-starts recording ── */}
                          <span
                            className="draft-name"
                            onClick={() => {
                              // Pass narrator name + next unrecorded page to BookPage
                              onNavigateToDraft(narrator, page_count + 1);
                            }}
                            style={{ cursor: 'pointer', textDecoration: 'underline' }}
                          >
                            Draft: <span className="boldened-text">{narrator}</span>{" "}
                            ({page_count}/{totalPages} pages) — tap to continue
                          </span>
                        </motion.div>
                      ))
                    ) : (
