...
const SubscriptionModal = ({ show, onClose, onPaymentSuccess }) => {
  ...
  // Missing error handler function - ADDED
  ...
  useEffect(() => {
    if (show && InAppPurchase && !initialized) {
      initializeIAP();
    }
  }, [show, initialized]);
  ...
  const initializeIAP = async () => {
    try {
      ...
      if (!InAppPurchase || typeof InAppPurchase.register !== 'function') {
        console.error('StoreKit not available');
        setError('In-app purchases not supported on this device');
        setInitialized(true);
        return;
      }
      ...
      // Register products with validation
      ...
      console.log('Registering products:', productsToRegister);
      await InAppPurchase.register(productsToRegister);
      ...
      InAppPurchase.when(PRODUCT_IDS.monthly).approved(finishPurchase);
      InAppPurchase.when(PRODUCT_IDS.yearly).approved(finishPurchase);
      InAppPurchase.when("error").then(handleIAPError);

      // Refresh with better error handling
      try {
        await InAppPurchase.refresh();
        console.log('IAP refresh completed');
      } catch (refreshError) {
        console.warn('IAP refresh had issues:', refreshError);
        // Continue anyway - products might still load
      }
      ...
      setInitialized(true); // Allow retry
    }
  };
  ...
  const updateProductsList = () => {
    try {
      ...
      // If no products available, show appropriate error
      if (availableProducts.length === 0) {
        setError('Subscription plans not currently available. Please try again later.');
      }
    } catch (err) {
  ...
  const verifyReceipt = async (product) => {
    try {
      ...
      // Get receipt data - different methods for different platforms
      let receiptData;
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      // Initiate purchase with error handling
